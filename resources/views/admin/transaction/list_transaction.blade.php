        ...
        <div class="card-body">
            <div class="analytic">
                <form action="" class="form-transaction">
                    <a class="all-list-transaction active-transaction" href="{{route('admin.transaction')}}">Đang hoạt động  ({{ $count_transactions}})</a>
                    <a href="{{route('admin.trash.transaction')}}">Vô hiệu hóa  ({{$count_trash_transactions}})</a>
                    <select id="status-transaction" name="status" style="color: #007bff; cursor: pointer; padding: 2px 0px; border-radius: 5px;">
                        <option value="0" disabled selected>Chọn trạng thái</option>
                        <option {{Request::get('status') == 'đang xử lý' ? "selected='selected'" : ""}} value="đang xử lý">Đang xử lý  ({{$count_status_processing}})</option>
                        <option {{Request::get('status') == 'đang vận chuyển' ? "selected='selected'" : ""}} value="đang vận chuyển">Đang vận chuyển  ({{$count_status_transport}})</option>
                        <option {{Request::get('status') == 'hoàn thành' ? "selected='selected'" : ""}} value="hoàn thành">Thành công  ({{$count_status_success}})</option>
                        <option {{Request::get('status') == 'hủy bỏ' ? "selected='selected'" : ""}} value="hủy bỏ">Đã hủy  ({{$count_status_cancel}})</option>
                    </select>
                </form>
            </div>
